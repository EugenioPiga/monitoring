...
PARENT_YEAR_NEW_COLS = [
    ...
    "ai_positions_any_strict_share", "ai_positions_any_broad_share", "ai_position_log1p",
    ...
    "ai_postings_any_strict_share", "ai_postings_any_broad_share", "ai_posting_log1p",
...
def parse_args():
    p = argparse.ArgumentParser(description="Build parent_rcid x occupation x year panel with promotions and AI/HR outcomes.")
...
def main():
    ...
    ensure_dir(diag_dir)
    spark = create_spark("build_parent_occupation_year_v2", args.threads, args.shuffle_partitions, args.tmpdir)
    ...
    # Promotion: worker remains in the same parent firm from t-1 to t and seniority rises.
    if "avg_seniority" in df.columns:
        w = Window.partitionBy("user_id").orderBy("year")
        df = (
            df
            .withColumn("lag_year", F.lag("year").over(w))
            .withColumn("lag_parent_rcid", F.lag("parent_rcid").over(w))
            .withColumn("lag_seniority", F.lag("avg_seniority").over(w))
            .withColumn(
                "continuing_same_parent",
                F.when((F.col("lag_year") == F.col("year") - 1) & (F.col("lag_parent_rcid") == F.col("parent_rcid")), F.lit(1.0)).otherwise(F.lit(0.0)),
            )
            .withColumn(
                "promotion_flag",
                F.when((F.col("continuing_same_parent") == 1.0) & F.col("avg_seniority").isNotNull() & F.col("lag_seniority").isNotNull() & (F.col("avg_seniority") > F.col("lag_seniority")), F.lit(1.0)).otherwise(F.lit(0.0)),
            )
        )
    else:
        df = df.withColumn("continuing_same_parent", F.lit(None).cast("double"))
        df = df.withColumn("promotion_flag", F.lit(None).cast("double"))
    ...
    agg = [
        ...
        F.sum(F.coalesce(F.col("promotion_flag"), F.lit(0.0)) * F.col("weight")).alias("n_promotions"),
        F.sum(F.coalesce(F.col("continuing_same_parent"), F.lit(0.0)) * F.col("weight")).alias("n_continuing_workers"),
    ]
    ...
    panel = panel.withColumn("promotion_rate", safe_divide(F.col("n_promotions"), F.col("n_workers")))
    panel = panel.withColumn("promotion_rate_continuers", safe_divide(F.col("n_promotions"), F.col("n_continuing_workers")))
    ...
    if "first_ai_posting_year_any_strict" in panel.columns:
        panel = panel.withColumn("event_time_ai_posting_strict", F.when(F.col("first_ai_posting_year_any_strict").isNotNull(), F.col("year") - F.col("first_ai_posting_year_any_strict")))
    if "first_ai_firm_year_any_strict" in panel.columns:
        panel = panel.withColumn("event_time_ai_firm_strict", F.when(F.col("first_ai_firm_year_any_strict").isNotNull(), F.col("year") - F.col("first_ai_firm_year_any_strict")))

    # Backward compatible log intensities if parent-year did not already create them.
    if "ai_position_log1p" not in panel.columns and "ai_positions_any_strict_weighted" in panel.columns:
        panel = panel.withColumn("ai_position_log1p", F.log1p(F.coalesce(F.col("ai_positions_any_strict_weighted"), F.lit(0.0))))
    if "ai_posting_log1p" not in panel.columns and "ai_postings_any_strict" in panel.columns:
        panel = panel.withColumn("ai_posting_log1p", F.log1p(F.coalesce(F.col("ai_postings_any_strict"), F.lit(0.0))))
    ...
    meta = {
        ...
        "promotion_definition": "same parent_rcid in t and t-1 and avg_seniority_t > avg_seniority_t_minus_1",
    }
    ...
    (
        ...
        .agg(
            ...
            F.avg("promotion_rate").alias("mean_promotion_rate"),
            F.avg("promotion_rate_continuers").alias("mean_promotion_rate_continuers"),
    ...
    (
        written.where(F.col("n_promotions") > 0)
        .select("parent_rcid", "occupation", "year", "n_promotions", "n_continuing_workers", "promotion_rate_continuers", "avg_seniority")
        .orderBy(F.desc("n_promotions"))
        .limit(1000)
        .coalesce(1).write.mode("overwrite").option("header", True)
        .csv(os.path.join(diag_dir, "02_promotion_examples_csv"))
    )

    print("[INFO] Done parent-occupation-year panel v2")
    print(meta)
    spark.stop()
